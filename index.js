$(document).ready(function() {
    ...
    var playlistData = [
      ...
      var playlistDataWrapper=document.getElementById("playlist-wrapper");
     ...
     function createPlayerCard(id){
        for(var i=0;i<videoPlaySectionData.length;i++){
            if(videoPlaySectionData[i].id==id){
                videoPlay.src="https://player.vimeo.com/video/"+videoPlaySectionData[i].vimeoId;
                viewsCount.innerText=videoPlaySectionData[i].views;
                videoTitle.innerText=videoPlaySectionData[i].title;
                videoDescription.innerText=videoPlaySectionData[i].description;
                $(".playlist-card").removeClass("active-card");
                $("#card"+id).addClass("active-card");
            }
        }
            document.body.scrollTop=0;//safari
            document.documentElement.scrollTop=0;//chrome,opera,IE and firefox
      ...
      function createPlaylistCard(obj, pos) {
        ...
        if(pos==0){
            ...
            createPlayerCard((pos+1));
        }
        mainDiv.addEventListener("click",function(){
            createPlayerCard(obj.id);
        })
    ...
    for(var i=0;i<playlistData.length;i++){
        playlistDataWrapper.appendChild(createPlaylistCard(playlistData[i],i));
    }
});